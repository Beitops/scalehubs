...
export const LeadsHistorialList = ({
  ...
  const getActionMenuItems = (lead: Lead) => {
    const items: { label: string; icon: React.ReactNode; onClick: () => void; className?: string }[] = [
      {
        label: 'Ver detalles',
        icon: (
    ...
    // Solo mostrar "Cancelar Estado" si estado_temporal es diferente de 'sin_tratar'
    if (lead.estado_temporal && lead.estado_temporal !== 'sin_tratar') {
      items.push({
